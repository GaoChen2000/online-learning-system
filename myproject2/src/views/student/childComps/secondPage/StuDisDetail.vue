...
import {getStuDisDetail, submitStuDis} from '@/network/student.js'

export default {
  name: 'StuDisDetail',
  created() {
    // 获取路由参数中的 dno
    this.dno=Number(this.$route.query.dno)
    
    // 发送请求，获得 dno 对应的讨论题的内容（请求返回一个对象）
    this.sno=sessionStorage.getItem('no')
    getStuDisDetail(this.dno,this.sno).then(res => {
      let obj=res.data
      // 将请求的数据，赋值给 data 数据
  ...
  methods: {
    join() {
      ...
      if(/^\s*$/.test(this.textarea)) {
        ...
        // 发送请求，提交数据
        submitStuDis(this.sno, this.dno, this.textarea).then(res => {
